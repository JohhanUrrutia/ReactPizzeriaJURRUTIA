    ...
    return(
        ...
        {Pizzas.map((elemento) => {  
            ...
            return  <div key={elemento.id} className="card-pizza p-3">
                        <div key={elemento.id} className="card bg-success">
                            ...
                            <div className="d-flex flex-column align-items-center">
                                <p className="m-0 texto-precio-antes text-white-50">ANTES ${elemento.price +5000}</p>
                                <h5 className="texto-precio m-0 text-white">${elemento.price}</h5>
                                <p className="text-white-50">Incluye despacho a domicilio gratis.</p>
                            </div>
                            <div className="d-flex align-items-center justify-content-center gap-2 mb-3">

                            <button onClick={() => navigate(`/Pizzas/${index}`)} className="btn btn-light">VER MÁS</button>
                                <button onClick={() => {añadirPizza(elemento)
                                setTotalPizza(totalPizza + elemento.price)
                                }
                                    } className="btn btn-light">AÑADIR</button>
                            </div>
                        </div>
                    </div>
            })}
